...
if [ ${TOKUDB_HOT_BACKUP_BOLT_ON} == "YES" ]; then
    echo "`date` | Installing the backup plugin"
    $DB_DIR/bin/mysql --user=${MYSQL_USER} --socket=${MYSQL_SOCKET} test -e "install plugin tokudb_backup soname 'tokubackup.so';"
fi
...
#$DB_DIR/bin/mysql --user=${MYSQL_USER} --socket=${MYSQL_SOCKET} test < gauntlet_inno.sql

echo "`date` | Checking that backup is still running"
if [ ${TOKUDB_HOT_BACKUP_BOLT_ON} == "YES" ]; then
    $DB_DIR/bin/mysql --user=${MYSQL_USER} --socket=${MYSQL_SOCKET} test -e "show processlist" | grep "set tokudb_backup_dir" > /dev/null
else
